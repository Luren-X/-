...
public class AuthService {
    ...
    public AuthService(PersonRepository personRepository, UserRepository userRepository, UserTypeRepository userTypeRepository, StudentRepository studentRepository,AuthenticationManager authenticationManager, JwtService jwtService, PasswordEncoder encoder, ResourceLoader resourceLoader, DormitoryManagerRepository dormitoryManagerRepository) {
    ...
    public DataResponse registerUser(@Valid @RequestBody DataRequest dataRequest) {
        ...
        }else if("DORMITORYMANAGER".equals(role)) {
            p.setType("3");
            ut = userTypeRepository.findByName(EUserType.ROLE_DORMITORYMANAGER);
        }
        ...
        if("STUDENT".equals(role)) {
            ...
            DormitoryManager dm = new DormitoryManager();
            dm.setPerson(p);
            dormitoryManagerRepository.saveAndFlush(dm);
        }
        return CommonMethod.getReturnData(LoginControlUtil.getInstance().getValidateCodeDataMap());
    }
}
